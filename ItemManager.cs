            ...
            foreach (Item item in registeredItems)
            {
                ...
                if (item.Crafting.Stations.Count > 0)
                {
                    string localizedName = new Regex("['[\"\\]]")
                        .Replace(english.Localize(item.Prefab.GetComponent<ItemDrop>().m_itemData.m_shared.m_name), "")
                        .Trim();
                    int order = 0;

                    List<ConfigurationManagerAttributes> hideWhenNoneAttributes = new();

                    cfg.table = config(localizedName, "Crafting Station", item.Crafting.Stations.First().Table,
                    ...
                    cfg.customTable = config(localizedName, "Custom Crafting Station",
                        item.Crafting.Stations.First().custom ?? "",
                    ...
                    cfg.tableLevel = config(localizedName, "Crafting Station Level",
                        item.Crafting.Stations.First().level,
                    ...
                    {
                        cfg.maximumTableLevel = config(localizedName, "Maximum Crafting Station Level",
                            item.MaximumRequiredStationLevel == int.MaxValue
                                ? item.Crafting.Stations.First().level + item.Prefab.GetComponent<ItemDrop>().m_itemData
                                    .m_shared.m_maxQuality - 1
                    ...
                    cfg.craft = itemConfig("Crafting Costs",
                        new SerializedRequirements(item.RequiredItems.Requirements).ToString(),
                        $"Item costs to craft {localizedName}");
                    if (item.Prefab.GetComponent<ItemDrop>().m_itemData.m_shared.m_maxQuality > 1)
                    {
                        cfg.upgrade = itemConfig("Upgrading Costs",
                            new SerializedRequirements(item.RequiredUpgradeItems.Requirements).ToString(),
                            $"Item costs per level to upgrade {localizedName}");
                    }
        ...
        foreach (Item item in registeredItems)
        {
            ...
            foreach (CraftingStationConfig station in item.Crafting.Stations)
            {
                ...
                recipe.m_resources = SerializedRequirements.toPieceReqs(__instance,
                    cfg == null
                        ? new SerializedRequirements(item.RequiredItems.Requirements)
                        : new SerializedRequirements(cfg.craft.Value),
                    cfg == null
                        ? new SerializedRequirements(item.RequiredUpgradeItems.Requirements)
                        : new SerializedRequirements(cfg.upgrade?.Value ?? ""));
    ...
    internal static void Patch_MaximumRequiredStationLevel(Recipe __instance, ref int __result, int quality)
    {
        if (itemDropMap.TryGetValue(__instance.m_item, out Item item))
        {
            __result = Mathf.Min(Mathf.Max(1, __instance.m_minStationLevel) + (quality - 1),
                itemConfigs[item].maximumTableLevel?.Value ?? item.MaximumRequiredStationLevel);
        }
    }
